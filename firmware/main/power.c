...
static void power_fade_display_for_shutdown(void);
...
static void shutdown_completion_timer_cb(lv_timer_t *timer) {
  ...
  power_fade_display_for_shutdown();
...
static void power_fade_display_for_shutdown(void) {
  ...
  if (current_pwm > dim_pwm) {
    lcd_fade_backlight(current_pwm, dim_pwm, LCD_BACKLIGHT_FADE_DURATION_MS);
  }
}
...
static void set_bar_value(void *obj, int32_t v) {
  ...
  if (v >= 100) {
     ...
     * transition to a timer so LVGL has time to paint 100%. The display fade
     * starts from that timer, after the full bar is visible. */
